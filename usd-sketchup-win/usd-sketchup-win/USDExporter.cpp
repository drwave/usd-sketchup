...
static pxr::GfVec4d defaultBackFaceRGBA(198.0/255.0, 214.0/255.0, 224.0/255.0, 1.0);
...
USDExporter::_performExport(const std::string& skpSrc,
	...
	sprintf_s(buffer, "USD Export took %3.2lf secs\n", exportTime);
	_exportTimeSummary += std::string(buffer);
	if (texturesTime > 1.0) {
		sprintf_s(buffer, "\tTextures Export took %3.2lf secs\n", texturesTime);
		_exportTimeSummary += std::string(buffer);
	}
	if (componentsTime > 1.0) {
		sprintf_s(buffer, "\tComponents Export took %3.2lf secs\n", componentsTime);
		_exportTimeSummary += std::string(buffer);
	}
	if (geomTime > 1.0) {
		sprintf_s(buffer, "\tScene Geometry Export took %3.2lf secs\n", geomTime);
		_exportTimeSummary += std::string(buffer);
	}
	if (camerasTime > 1.0) {
		sprintf_s(buffer, "\tCameras Export took %3.2lf secs\n", camerasTime);
		_exportTimeSummary += std::string(buffer);
	}
	if (usdzTime > 1.0) {
		sprintf_s(buffer, "\tUSDZ Export took %3.2lf secs\n", usdzTime);
		_exportTimeSummary += std::string(buffer);
	}
...
USDExporter::_ExportInstance(const pxr::SdfPath parentPath,
	...
	if (SUIsValid(instanceMaterial)) {
		...
		if (SU_ERROR_NONE == SUMaterialGetTexture(instanceMaterial, &textureRef)) {
			...
			if (SUMaterialGetColor(instanceMaterial, &color) == SU_ERROR_NONE) {
				pxr::GfVec3f rgb;
...
USDExporter::_generateRGBAMaterialName(pxr::GfVec3f rgb, float opacity) {
	...
	sprintf_s(buffer, "RGBAMaterial_%.8f_%.8f_%.8f_%.8f",
...
USDExporter::_ExportCamera(const pxr::SdfPath parentPath, SUSceneRef scene) {
	...
	if (SUCameraGetAspectRatio(camera, &aspect_ratio) == SU_ERROR_NONE) {
